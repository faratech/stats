 ...
 * This version uses a writable copy of the DNF history database on disk.
 *
 * @version 1.2.0
...
// Configuration - paths for original and copied DNF history database
$original_db_path = "/web/stats/dnf/history.sqlite";
$copy_db_path = "/web/stats/dnf/history_copy.sqlite";

// Global variable to store the database connection
$db = null;

// Create a writable copy of the database if it doesn't exist
try {
    // Check original database
    if (!file_exists($original_db_path)) {
        throw new Exception("Original database file not found: $original_db_path");
    }
    if (!is_readable($original_db_path)) {
        throw new Exception("Original database file is not readable: $original_db_path");
    }

    // Check parent directory permissions
    $parent_dir = dirname($copy_db_path);
    if (!is_writable($parent_dir)) {
        throw new Exception("Parent directory is not writable: $parent_dir");
    }
    error_log("DNF Visualizer: Parent directory $parent_dir is writable");

    // Create copy if it doesn't exist
    if (!file_exists($copy_db_path)) {
        error_log("DNF Visualizer: Creating database copy: $copy_db_path");

        // Try simple file copy first
        if (!copy($original_db_path, $copy_db_path)) {
            error_log("DNF Visualizer: File copy failed, attempting VACUUM INTO");
            // Fallback to SQLite VACUUM INTO
            $temp_db = new SQLite3($original_db_path, SQLITE3_OPEN_READONLY);
            $temp_db->exec("VACUUM INTO '$copy_db_path'");
            $temp_db->close();
            if (!file_exists($copy_db_path)) {
                throw new Exception("Failed to create database copy with VACUUM INTO: $copy_db_path");
            }
        }

        // Set permissions to ensure writability
        if (!chmod($copy_db_path, 0666)) {
            throw new Exception("Failed to set permissions on database copy: $copy_db_path");
        }

        // Attempt to set ownership to nobody:nobody (suppress errors if it fails)
        @chown($copy_db_path, 'nobody');
        @chgrp($copy_db_path, 'nobody');

        error_log("DNF Visualizer: Database copy created and permissions set");
    }

    // Verify copy is accessible
    if (!is_readable($copy_db_path) || !is_writable($copy_db_path)) {
        throw new Exception("Database copy is not readable or writable: $copy_db_path");
    }

    // Connect to the copied database
    error_log("DNF Visualizer: Attempting to open database copy: $copy_db_path");
    $db = new SQLite3($copy_db_path, SQLITE3_OPEN_READWRITE);

    // Optimize database performance without write-heavy PRAGMAs
    $db->exec('PRAGMA synchronous = OFF');
    $db->exec('PRAGMA journal_mode = MEMORY'); // Use MEMORY to avoid disk writes
    $db->exec('PRAGMA cache_size = 10000');

    error_log("DNF Visualizer: Database copy opened successfully");
} catch (Exception $e) {
    ...
    if (isset($_GET['api'])) {
        ...
        echo json_encode(['error' => 'Exception connecting to database: ' . $e->getMessage(), 'db_path' => $copy_db_path]);
        exit;
    } else {
...
<body>
    ...
    <footer>
        ...
        <p><span id="data-source-info">Using package data from a copy of the DNF history database</span></p>
