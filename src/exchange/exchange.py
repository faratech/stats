...
def is_exchange_server():
    ...
    try:
        ...
        print(f"Error in is_exchange_server(): {e}")
...
def run_exchange_shell_command(command):
    """
    Executes an Exchange Management Shell command and returns the output.
    """
    try:
        # Prepare the command to run in PowerShell
        ps_command = f"Add-PSSnapin Microsoft.Exchange.Management.PowerShell.SnapIn; {command}"
...
async def collect_stats(previous_net_io):
    try:
        ...
        if static_data.get('is_exchange_server'):
            ...
            # Run Exchange Management Shell commands
            exchange_shell_output = run_exchange_shell_command("Get-Queue | ft -AutoSize")
            # Add to stats
            stats['exchange_services_status'] = exchange_services_status
            stats['exchange_logs'] = exchange_logs
            stats['exchange_shell_output'] = exchange_shell_output
...
async def index(request: Request):
    await initialize_static_data()  # Ensure static data is initialized
    page_title = "Exchange Monitoring System" if static_data.get('is_exchange_server') else "System Monitor"
